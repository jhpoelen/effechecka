...
trait OccurrenceCollectionFetcherHDFS
  ...
  private def graphForOccurrences(ocRequest: OccurrenceRequest) = {
    ...
    GraphDSL.create(new EelRowSourceShape(patternFor1, ocRequest.limit)) { implicit builder =>
  ...
  def monitoredOccurrencesFor(source: String,
    ...
    val aGraph = GraphDSL.create(new EelRowSourceShape(patternFor(s"source-of-monitored-occurrence/source=$source"), occLimit)) { implicit builder =>
      (rows) =>
        import GraphDSL.Implicits._
  ...
  def monitors(): List[OccurrenceMonitor] = {
    val aGraph = GraphDSL.create(new EelRowSourceShape(patternFor(s"occurrence-summary"), None)) { implicit builder =>
      (rows) =>
        import GraphDSL.Implicits._
  ...
  def monitorOf(selector: OccurrenceSelector): Option[OccurrenceMonitor] = {
    val aGraph = GraphDSL.create(new EelRowSourceShape(patternFor(s"occurrence-summary/${pathForSelector(selector)}"), Some(1))) { implicit builder =>
      (rows) =>
        import GraphDSL.Implicits._
        val toMonitors = Flow[Row]
          .map(row => {
